        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              Text('Sri Lanka-specific calculators & market data', style: TextStyle(color: Colors.white.withValues(alpha:0.5), fontSize: 14)),
              ...
              Padding(
                ...
                child: GestureDetector(
                  ...
                  child: Text('Admin', style: TextStyle(color: Colors.white.withValues(alpha:0.2), fontSize: 12)),
                ),
              ),
              const SizedBox(height: 8),
              Padding(
                padding: const EdgeInsets.only(top: 16, bottom: 12),
                child: Text('CALCULATORS', style: TextStyle(color: Colors.white.withValues(alpha:0.3), fontSize: 11, fontWeight: FontWeight.w700, letterSpacing: 2)),
              ),
              Row(children: [
              ...
              Padding(
                ...
                child: Text('MARKET DATA', style: TextStyle(color: Colors.white.withValues(alpha:0.3), fontSize: 11, fontWeight: FontWeight.w700, letterSpacing: 2)),
              ),
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: const Color(0xFF1A1A2E), borderRadius: BorderRadius.circular(18), border: Border.all(color: Colors.white.withValues(alpha:0.05))),
        child: isFullWidth
            ? Row(children: [
                Container(width: 48, height: 48, decoration: BoxDecoration(color: color.withValues(alpha:0.1), borderRadius: BorderRadius.circular(14)), child: Icon(icon, color: color, size: 24)),
                const SizedBox(width: 16),
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w600, fontSize: 15)), const SizedBox(height: 3), Text(subtitle, style: TextStyle(color: Colors.white.withValues(alpha:0.45), fontSize: 12))])),
                Icon(Icons.arrow_forward_ios, color: Colors.white.withValues(alpha:0.25), size: 16),
              ])
            : Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Text(subtitle, style: TextStyle(color: Colors.white.withValues(alpha:0.45), fontSize: 12, height: 1.3), maxLines: 2, overflow: TextOverflow.ellipsis),
              ]),
      ),
    );
  }
...
class _FDCalculatorScreenState extends State<FDCalculatorScreen> {
  ...
  try {
    ...
    for (var bank in fdRates.keys) {
      ...
      double grossInterest = _amount * rate * (months / 12);
      double whtAmount = grossInterest * 0.05;      // 5% Withholding Tax (Sri Lanka)
      double netInterest = grossInterest - whtAmount;
      double maturity = _amount + netInterest;

      // Correct EAR: compound annualisation, not linear scaling
      double effectiveAnnualRate = months < 12
          ? math.pow(1 + rate * months / 12, 12.0 / months).toDouble() - 1
          : rate;

      _bankResults[bank] = BankFDResult(
        bankName: bank, rate: rate,
        grossInterest: grossInterest, whtAmount: whtAmount,
        interest: netInterest, maturity: maturity,
        months: months, effectiveAnnualRate: effectiveAnnualRate,
      );
  ...
  Widget _buildBestBankCard() {
    ...
    return Container(padding: const EdgeInsets.all(24), decoration: BoxDecoration(gradient: LinearGradient(colors: [const Color(0xFF4CAF50).withValues(alpha:0.2), const Color(0xFF4CAF50).withValues(alpha:0.05)], begin: Alignment.topLeft, end: Alignment.bottomRight), borderRadius: BorderRadius.circular(20), border: Border.all(color: const Color(0xFF4CAF50).withValues(alpha:0.3))), child: Column(children: [
      ...
      const SizedBox(height: 16), Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
        ...
        _buildBestStat('Net Interest\n(after 5% WHT)', 'LKR ${NumberFormat('#,##0').format(best.interest)}'), Container(width: 1, height: 40, color: Colors.white10),
        _buildBestStat('Maturity', 'LKR ${NumberFormat('#,##0').format(best.maturity)}'),
      ]),
    ]));
  }
  Widget _buildBestStat(String l, String v) => Column(children: [Text(v, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 15)), const SizedBox(height: 2), Text(l, style: TextStyle(color: Colors.white.withValues(alpha:0.5), fontSize: 11))]);
  ...
  Widget _buildComparisonChart() {
    ...
    return Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: const Color(0xFF1A1A2E), borderRadius: BorderRadius.circular(20)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Text('$_selectedPeriod deposit of LKR ${NumberFormat('#,##0').format(_amount)}', style: TextStyle(color: Colors.white.withValues(alpha:0.5), fontSize: 13)), const SizedBox(height: 20),
      ...sorted.map((entry) { double pct = (entry.value.maturity / maxMaturity * 100); bool isBest = entry.key == _bestBank; return Padding(padding: const EdgeInsets.only(bottom: 14), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Row(children: [if (isBest) const Icon(Icons.star, color: Color(0xFFFFD700), size: 16), if (isBest) const SizedBox(width: 4), Text(entry.key, style: TextStyle(color: isBest ? const Color(0xFFFFD700) : Colors.white, fontWeight: FontWeight.w600, fontSize: 13))]), Text('LKR ${NumberFormat('#,##0').format(entry.value.maturity)}', style: TextStyle(color: isBest ? const Color(0xFFFFD700) : const Color(0xFF42A5F5), fontWeight: FontWeight.bold, fontSize: 13))]), const SizedBox(height: 6), ClipRRect(borderRadius: BorderRadius.circular(4), child: LinearProgressIndicator(value: pct / 100, backgroundColor: Colors.white.withValues(alpha:0.05), valueColor: AlwaysStoppedAnimation<Color>(isBest ? const Color(0xFFFFD700) : const Color(0xFF42A5F5)), minHeight: 6))])); }),
    ]));
  }

  Widget _buildAllBanksList() {
    var sorted = _bankResults.entries.toList()..sort((a, b) => b.value.maturity.compareTo(a.value.maturity));
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('All Bank Rates', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)), const SizedBox(height: 4), Text('Tap a bank for full details', style: TextStyle(color: Colors.white.withValues(alpha:0.4), fontSize: 12)), const SizedBox(height: 12), ...sorted.map((entry) { bool isSel = _selectedDetailBank == entry.key; bool isBest = entry.key == _bestBank; return GestureDetector(onTap: () => setState(() => _selectedDetailBank = isSel ? null : entry.key), child: Container(margin: const EdgeInsets.only(bottom: 8), padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: const Color(0xFF1A1A2E), borderRadius: BorderRadius.circular(14), border: Border.all(color: isSel ? const Color(0xFF42A5F5) : Colors.white.withValues(alpha:0.05), width: isSel ? 1.5 : 1)), child: Row(children: [if (isBest) Container(margin: const EdgeInsets.only(right: 8), padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2), decoration: BoxDecoration(color: const Color(0xFFFFD700).withValues(alpha:0.15), borderRadius: BorderRadius.circular(6)), child: const Text('Best', style: TextStyle(color: Color(0xFFFFD700), fontSize: 9, fontWeight: FontWeight.bold))), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(entry.key, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w600, fontSize: 14)), const SizedBox(height: 2), Text('${(entry.value.rate * 100).toStringAsFixed(2)}% p.a. • $_selectedPeriod', style: TextStyle(color: Colors.white.withValues(alpha:0.4), fontSize: 11))])), Column(crossAxisAlignment: CrossAxisAlignment.end, children: [Text('LKR ${NumberFormat('#,##0').format(entry.value.maturity)}', style: const TextStyle(color: Color(0xFF42A5F5), fontWeight: FontWeight.bold, fontSize: 14)), const SizedBox(height: 2), Text('+LKR ${NumberFormat('#,##0').format(entry.value.interest)}', style: TextStyle(color: const Color(0xFF4CAF50).withValues(alpha:0.7), fontSize: 11))])]))); })]);
  }

  Widget _buildBankDetailCard() {
    final d = _bankResults[_selectedDetailBank]; if (d == null) return const SizedBox();
    return Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: const Color(0xFF1A1A2E), borderRadius: BorderRadius.circular(20), border: Border.all(color: const Color(0xFF42A5F5).withValues(alpha: 0.3))), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [const Icon(Icons.account_balance, color: Color(0xFF42A5F5), size: 20), const SizedBox(width: 8), Text(d.bankName, style: const TextStyle(color: Color(0xFF42A5F5), fontSize: 18, fontWeight: FontWeight.bold))]), const SizedBox(height: 16), _buildDRow('Investment Amount', 'LKR ${NumberFormat('#,##0.00').format(_amount)}'), _buildDRow('Interest Rate', '${(d.rate * 100).toStringAsFixed(2)}% p.a.'), _buildDRow('Deposit Period', _selectedPeriod), _buildDRow('Effective Annual Rate (net)', '${(d.effectiveAnnualRate * 95 / 100).toStringAsFixed(2)}%'), const Divider(color: Colors.white10, height: 20), _buildDRow('Gross Interest', 'LKR ${NumberFormat('#,##0.00').format(d.grossInterest)}'), _buildDRow('WHT Deducted (5%)', '- LKR ${NumberFormat('#,##0.00').format(d.whtAmount)}', c: const Color(0xFFEF5350)), _buildDRow('Net Interest Earned', 'LKR ${NumberFormat('#,##0.00').format(d.interest)}', hl: true, c: const Color(0xFF4CAF50)), _buildDRow('Maturity Amount', 'LKR ${NumberFormat('#,##0.00').format(d.maturity)}', hl: true, c: const Color(0xFF42A5F5))]));
  }
  Widget _buildDRow(String l, String v, {bool hl = false, Color? c}) => Padding(padding: const EdgeInsets.symmetric(vertical: 5), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(l, style: TextStyle(color: Colors.white.withValues(alpha:0.6), fontSize: 13)), Text(v, style: TextStyle(color: c ?? Colors.white, fontWeight: hl ? FontWeight.bold : FontWeight.normal, fontSize: 13))]));

  Widget _buildHowItWorks() {
    return Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: const Color(0xFF1A1A2E), borderRadius: BorderRadius.circular(20)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: const Color(0xFF42A5F5).withValues(alpha:0.1), borderRadius: BorderRadius.circular(8)), child: const Icon(Icons.lightbulb_outline, color: Color(0xFF42A5F5), size: 18)), const SizedBox(width: 10), const Text('How Fixed Deposits Work', style: TextStyle(color: Colors.white, fontSize: 17, fontWeight: FontWeight.bold))]), const SizedBox(height: 16), Text('A Fixed Deposit (FD) is a savings instrument where you deposit a lump sum for a fixed period at a guaranteed interest rate.', style: TextStyle(color: Colors.white.withValues(alpha:0.65), fontSize: 13, height: 1.6)), const SizedBox(height: 16), _buildStep('1', 'Choose Your Deposit', 'Decide how much to invest and for how long.'), _buildStep('2', 'Select a Bank', 'Compare rates across banks.'), _buildStep('3', 'Earn Guaranteed Returns', 'Your interest rate is locked.'), _buildStep('4', 'Receive Maturity Amount', 'Get principal plus interest at end of term.')]));
  }
  Widget _buildStep(String n, String t, String d) => Padding(padding: const EdgeInsets.only(bottom: 14), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [Container(width: 28, height: 28, decoration: BoxDecoration(color: const Color(0xFF42A5F5).withValues(alpha:0.15), borderRadius: BorderRadius.circular(8)), child: Center(child: Text(n, style: const TextStyle(color: Color(0xFF42A5F5), fontWeight: FontWeight.bold, fontSize: 13)))), const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(t, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w600, fontSize: 13)), const SizedBox(height: 2), Text(d, style: TextStyle(color: Colors.white.withValues(alpha:0.55), fontSize: 12, height: 1.4))]))]));

  Widget _buildCalculationFormula() {
    return Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(gradient: LinearGradient(colors: [const Color(0xFF42A5F5).withValues(alpha:0.1), const Color(0xFF42A5F5).withValues(alpha:0.03)], begin: Alignment.topLeft, end: Alignment.bottomRight), borderRadius: BorderRadius.circular(20), border: Border.all(color: const Color(0xFF42A5F5).withValues(alpha:0.15))), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [const Icon(Icons.calculate, color: Color(0xFF42A5F5), size: 20), const SizedBox(width: 8), const Text('How We Calculate Returns', style: TextStyle(color: Color(0xFF42A5F5), fontSize: 17, fontWeight: FontWeight.bold))]), const SizedBox(height: 16), Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: const Color(0xFF0B0C10), borderRadius: BorderRadius.circular(12)), child: Column(children: [const Text('FD Return Formula (with 5% WHT)', style: TextStyle(color: Colors.white70, fontSize: 12, fontWeight: FontWeight.w600)), const SizedBox(height: 8), const Text('Gross Interest = Principal × Rate × (Months/12)', style: TextStyle(color: Color(0xFF42A5F5), fontSize: 12, fontWeight: FontWeight.w600, fontFamily: 'monospace')), const SizedBox(height: 4), const Text('WHT = Gross Interest × 5%', style: TextStyle(color: Color(0xFFEF5350), fontSize: 12, fontWeight: FontWeight.w600, fontFamily: 'monospace')), const SizedBox(height: 4), const Text('Net Interest = Gross Interest − WHT', style: TextStyle(color: Color(0xFF4CAF50), fontSize: 12, fontWeight: FontWeight.w600, fontFamily: 'monospace')), const SizedBox(height: 4), const Text('Maturity = Principal + Net Interest', style: TextStyle(color: Color(0xFF42A5F5), fontSize: 12, fontFamily: 'monospace'))])), const SizedBox(height: 12), Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: const Color(0xFF0B0C10), borderRadius: BorderRadius.circular(12)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Example: LKR 100,000 at 10% for 1 Year', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w600, fontSize: 12)), const SizedBox(height: 6), const Text('Gross Interest = LKR 10,000', style: TextStyle(color: Colors.white70, fontSize: 12)), const Text('WHT (5%) = LKR 500', style: TextStyle(color: Color(0xFFEF5350), fontSize: 12, fontWeight: FontWeight.w600)), const Text('Net Interest = LKR 9,500', style: TextStyle(color: Color(0xFF4CAF50), fontSize: 12, fontWeight: FontWeight.w600)), const Text('Maturity = LKR 109,500', style: TextStyle(color: Color(0xFF42A5F5), fontSize: 12, fontWeight: FontWeight.w600))]))]));
  }

  Widget _buildDisclaimer() {
    return Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: const Color(0xFFFFA726).withValues(alpha:0.05), borderRadius: BorderRadius.circular(16), border: Border.all(color: const Color(0xFFFFA726).withValues(alpha:0.15))), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [const Icon(Icons.warning_amber_rounded, color: Color(0xFFFFA726), size: 20), const SizedBox(width: 10), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Important Notice', style: TextStyle(color: Color(0xFFFFA726), fontWeight: FontWeight.bold, fontSize: 13)), const SizedBox(height: 4), Text('Rates are indicative. Please contact your bank for exact rates. FDs insured up to Rs. 1,100,000 under Sri Lanka Deposit Insurance Scheme.', style: TextStyle(color: Colors.white.withValues(alpha:0.6), fontSize: 11, height: 1.5))]))]));
  }
}
...
class _LeasingCalculatorScreenState extends State<LeasingCalculatorScreen> {
  ...
  bool _isCalculated = false;
  // Cached config data — used by display widgets to stay in sync with calculations
  Map<String, VehicleLTV> _ltvData = SLFinancialData.ltvRatios;
  Map<String, Map<int, double>> _leasingRates = SLFinancialData.bankLeasingRates;
  ...
  // Cache for display widgets so they use the same source as the calculation
  _ltvData = ltvData;
  _leasingRates = leasingRates;
  ...
  // Use the fetched config rates, not hardcoded SLFinancialData
  _interestRate = leasingRates[_selectedBank]?[_selectedTenure] ?? 0.14;
  _loanAmount = (maxLoan - lumpSum).clamp(0, double.infinity);

  if (lumpSum > maxLoan && mounted) {
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
      content: Text('Lump sum exceeds the maximum loan amount. Monthly rental will be zero.'),
  ...
  // Correct formula: total interest = sum of all monthly rental payments minus the principal lent
  _totalInterest = _monthlyRental * _selectedTenure - _loanAmount;
  setState(() => _isCalculated = true);
  }
  ...
  Widget _buildCBSLNotice() {
    return Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: const Color(0xFFFFA726).withValues(alpha:0.08), borderRadius: BorderRadius.circular(14), border: Border.all(color: const Color(0xFFFFA726).withValues(alpha:0.2))), child: Row(children: [const Icon(Icons.gavel, color: Color(0xFFFFA726), size: 20), const SizedBox(width: 10), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('CBSL Regulation Update', style: TextStyle(color: Color(0xFFFFA726), fontWeight: FontWeight.bold, fontSize: 13)), const SizedBox(height: 2), Text('New LTV ratios effective May 25, 2026. Unregistered vehicles: 40% LTV. Registered vehicles: 70% LTV.', style: TextStyle(color: Colors.white.withValues(alpha:0.7), fontSize: 11, height: 1.4))]))]));
  }
  ...
  Widget _buildLTVDisplay() {
    ...
    final ltvData = _ltvData[_selectedVehicle];
    double ltv = ltvData?.ltv ?? 0.40;
    double maxLoan = value * ltv;
    double minDownPayment = value - maxLoan;
    return Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: const Color(0xFF66FCF1).withValues(alpha:0.05), borderRadius: BorderRadius.circular(12), border: Border.all(color: const Color(0xFF66FCF1).withValues(alpha:0.1))), child: Column(children: [
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text('LTV Ratio (${ltvData?.category ?? 'Standard'})', style: TextStyle(color: Colors.white.withValues(alpha:0.6), fontSize: 12)), Text('${(ltv * 100).toStringAsFixed(0)}%', style: const TextStyle(color: Color(0xFF66FCF1), fontWeight: FontWeight.bold, fontSize: 14))]),
      const SizedBox(height: 6), Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text('Min. Down Payment Required', style: TextStyle(color: Colors.white.withValues(alpha:0.6), fontSize: 12)), Text('LKR ${NumberFormat('#,##0').format(minDownPayment)}', style: const TextStyle(color: Color(0xFFEF5350), fontWeight: FontWeight.bold, fontSize: 14))]),
      const SizedBox(height: 6), Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text('Max Loan Amount', style: TextStyle(color: Colors.white.withValues(alpha:0.6), fontSize: 12)), Text('LKR ${NumberFormat('#,##0').format(maxLoan)}', style: const TextStyle(color: Color(0xFF4CAF50), fontWeight: FontWeight.bold, fontSize: 14))]),
    ]));
  }

  Widget _buildInterestRateDisplay() {
    double rate = _leasingRates[_selectedBank]?[_selectedTenure] ?? 0.14;
    return Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: const Color(0xFF66FCF1).withValues(alpha:0.05), borderRadius: BorderRadius.circular(12)), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text('Interest Rate ($_selectedBank)', style: TextStyle(color: Colors.white.withValues(alpha:0.6), fontSize: 12)), Text('${(rate * 100).toStringAsFixed(1)}% p.a.', style: const TextStyle(color: Color(0xFF66FCF1), fontWeight: FontWeight.bold, fontSize: 14))]));
  }

  Widget _buildTenureSelector() {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Tenure: $_selectedTenure months', style: TextStyle(color: Colors.white.withValues(alpha:0.6), fontSize: 12)), const SizedBox(height: 8), Wrap(spacing: 8, children: SLFinancialData.tenureOptions.map((t) { bool isSelected = _selectedTenure == t; return ChoiceChip(label: Text('${t}mo', style: TextStyle(fontSize: 12, color: isSelected ? const Color(0xFF0B0C10) : Colors.white70)), selected: isSelected, onSelected: (_) { setState(() => _selectedTenure = t); _updateCalculations(); }, selectedColor: const Color(0xFF66FCF1), backgroundColor: const Color(0xFF0B0C10), side: BorderSide(color: isSelected ? const Color(0xFF66FCF1) : Colors.white.withValues(alpha:0.1))); }).toList())]);
  }
  ...
  Widget _buildResultCard(String label, String value, Color color, IconData icon) => Expanded(child: Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: const Color(0xFF1A1A2E), borderRadius: BorderRadius.circular(16)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: color.withValues(alpha:0.1), borderRadius: BorderRadius.circular(8)), child: Icon(icon, color: color, size: 16)), const SizedBox(height: 12), Text(value, style: TextStyle(color: color, fontWeight: FontWeight.bold, fontSize: 16)), const SizedBox(height: 2), Text(label, style: TextStyle(color: Colors.white.withValues(alpha:0.5), fontSize: 11))])));

  Widget _buildSummary() => Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(gradient: LinearGradient(colors: [const Color(0xFF66FCF1).withValues(alpha:0.1), const Color(0xFF45A29E).withValues(alpha:0.05)], begin: Alignment.topLeft, end: Alignment.bottomRight), borderRadius: BorderRadius.circular(20), border: Border.all(color: const Color(0xFF66FCF1).withValues(alpha:0.15))), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Row(children: [Icon(Icons.description, color: Color(0xFF66FCF1), size: 20), SizedBox(width: 8), Text('Loan Details', style: TextStyle(color: Color(0xFF66FCF1), fontSize: 18, fontWeight: FontWeight.bold))]), const SizedBox(height: 16), _buildSummaryRow('Vehicle', _selectedVehicle), _buildSummaryRow('Bank', _selectedBank), _buildSummaryRow('Tenure', '$_selectedTenure months'), _buildSummaryRow('Interest Rate', '${(_interestRate * 100).toStringAsFixed(1)}% p.a.'), _buildSummaryRow('LTV Ratio', '${(_ltvRatio * 100).toStringAsFixed(0)}%'), _buildSummaryRow('Monthly Rental', 'LKR ${NumberFormat('#,##0.00').format(_monthlyRental)}', hl: true)]));

  Widget _buildSummaryRow(String l, String v, {bool hl = false}) => Padding(padding: const EdgeInsets.symmetric(vertical: 4), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(l, style: TextStyle(color: Colors.white.withValues(alpha:0.7), fontSize: 13)), Text(v, style: TextStyle(color: hl ? const Color(0xFF66FCF1) : Colors.white, fontWeight: hl ? FontWeight.bold : FontWeight.normal, fontSize: 13))]));

  Widget _buildLTVInfo() => Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: const Color(0xFFFFA726).withValues(alpha:0.05), borderRadius: BorderRadius.circular(16), border: Border.all(color: const Color(0xFFFFA726).withValues(alpha:0.15))), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Row(children: [Icon(Icons.info_outline, color: Color(0xFFFFA726), size: 18), SizedBox(width: 8), Text('CBSL LTV Regulations', style: TextStyle(color: Color(0xFFFFA726), fontWeight: FontWeight.bold, fontSize: 14))]), const SizedBox(height: 8), _buildLTVRow('Unregistered Vehicles', '40% LTV'), _buildLTVRow('Registered Vehicles (>1 year)', '70% LTV'), _buildLTVRow('Electric Vehicles', '60% LTV'), _buildLTVRow('Commercial Vehicles', '60% LTV'), const SizedBox(height: 8), Text('Effective from May 25, 2026 per CBSL directive.', style: TextStyle(color: Colors.white.withValues(alpha:0.5), fontSize: 11))]));
  Widget _buildLTVRow(String t, String l) => Padding(padding: const EdgeInsets.symmetric(vertical: 3), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(t, style: TextStyle(color: Colors.white.withValues(alpha:0.7), fontSize: 12)), Text(l, style: const TextStyle(color: Color(0xFFFFA726), fontWeight: FontWeight.bold, fontSize: 12))]));
}
...
class _LoanCalculatorScreenState extends State<LoanCalculatorScreen> {
  ...
  Widget _buildInputSection(LoanType loanType, List<int> tenures) => Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: const Color(0xFF1A1A2E), borderRadius: BorderRadius.circular(20), border: Border.all(color: Colors.white.withValues(alpha:0.05))), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: const Color(0xFFAB47BC).withValues(alpha:0.1), borderRadius: BorderRadius.circular(12)), child: const Icon(Icons.monetization_on_rounded, color: Color(0xFFAB47BC), size: 22)), const SizedBox(width: 12), const Text('Loan Details', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold))]), const SizedBox(height: 20), const Text('Loan Type:', style: TextStyle(color: Colors.white70, fontSize: 13)), const SizedBox(height: 10), Wrap(spacing: 8, runSpacing: 8, children: SLFinancialData.loanTypes.map((type) { bool isSelected = _selectedLoanType == type.name; return ChoiceChip(label: Row(mainAxisSize: MainAxisSize.min, children: [Icon(_getLoanIcon(type.name), size: 16, color: isSelected ? Colors.white : Colors.white60), const SizedBox(width: 6), Text(type.name, style: TextStyle(fontSize: 12, color: isSelected ? Colors.white : Colors.white70))]), selected: isSelected, onSelected: (_) => _onLoanTypeChanged(type.name), selectedColor: const Color(0xFFAB47BC), backgroundColor: const Color(0xFF0B0C10), side: BorderSide(color: isSelected ? const Color(0xFFAB47BC) : Colors.white.withValues(alpha:0.1))); }).toList()), const SizedBox(height: 8), Text(loanType.description, style: TextStyle(color: Colors.white.withValues(alpha:0.4), fontSize: 11)), const SizedBox(height: 16), _buildDropdown('Bank', _selectedBank, SLFinancialData.loanBanks, _onBankChanged), const SizedBox(height: 14), _buildTextField('Loan Amount (LKR)', _amountController, hint: 'e.g., 1,000,000'), const SizedBox(height: 14), Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: const Color(0xFFAB47BC).withValues(alpha:0.05), borderRadius: BorderRadius.circular(12)), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text('Interest Rate ($_selectedBank)', style: TextStyle(color: Colors.white.withValues(alpha:0.6), fontSize: 12)), Text('${(_interestRate * 100).toStringAsFixed(1)}% p.a.', style: const TextStyle(color: Color(0xFFAB47BC), fontWeight: FontWeight.bold, fontSize: 14))])), const SizedBox(height: 14), const Text('Tenure:', style: TextStyle(color: Colors.white70, fontSize: 13)), const SizedBox(height: 8), Wrap(spacing: 8, children: tenures.map((t) { bool isSelected = _selectedTenure == t; String label = t >= 12 ? '${t ~/ 12}Y' : '${t}M'; return ChoiceChip(label: Text(label, style: TextStyle(fontSize: 12, color: isSelected ? Colors.white : Colors.white70)), selected: isSelected, onSelected: (_) => setState(() => _selectedTenure = t), selectedColor: const Color(0xFFAB47BC), backgroundColor: const Color(0xFF0B0C10), side: BorderSide(color: isSelected ? const Color(0xFFAB47BC) : Colors.white.withValues(alpha:0.1))); }).toList()), const SizedBox(height: 14), _buildTextField('Initial Lump Sum Payment (LKR)', _lumpSumController, hint: 'Optional'), const SizedBox(height: 14), _buildTextField('Additional Monthly Payment (LKR)', _recurringLumpSumController, hint: 'Optional - reduces tenure')]));
  ...
  Widget _buildResultCard(String l, String v, Color c, IconData i) => Expanded(child: Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: const Color(0xFF1A1A2E), borderRadius: BorderRadius.circular(16)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: c.withValues(alpha:0.1), borderRadius: BorderRadius.circular(8)), child: Icon(i, color: c, size: 16)), const SizedBox(height: 12), Text(v, style: TextStyle(color: c, fontWeight: FontWeight.bold, fontSize: 15)), const SizedBox(height: 2), Text(l, style: TextStyle(color: Colors.white.withValues(alpha:0.5), fontSize: 11))])));
        ...
        children: [
          ...
          ...sorted.map((e) {
            ...
            return Container(
              ...
              decoration: BoxDecoration(
                color: isBest ? const Color(0xFF4CAF50).withValues(alpha:0.05) : const Color(0xFF0B0C10),
                borderRadius: BorderRadius.circular(12),
                border: Border.all(
                  color: isBest ? const Color(0xFF4CAF50).withValues(alpha:0.3) : Colors.white.withValues(alpha:0.05),
                ),
              ),
              child: Row(
                children: [
  ...
  Widget _buildAmortizationSection() => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text('Payment Schedule', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)), TextButton.icon(icon: Icon(_showSchedule ? Icons.expand_less : Icons.expand_more, color: const Color(0xFFAB47BC)), label: Text(_showSchedule ? 'Hide' : 'Show Full Schedule', style: const TextStyle(color: Color(0xFFAB47BC))), onPressed: () => setState(() => _showSchedule = !_showSchedule))]), if (_showSchedule) ...[const SizedBox(height: 8), SizedBox(height: 120, child: Row(crossAxisAlignment: CrossAxisAlignment.end, children: [...List.generate(_schedule.length.clamp(0, 24), (i) { if (i % (_schedule.length ~/ 12).clamp(1, 100) != 0 && i != 0 && i != _schedule.length - 1) return const SizedBox.shrink(); double mv = _schedule.map((e) => e.emi).reduce((a, b) => a > b ? a : b); double pct = mv > 0 ? _schedule[i].emi / mv : 0; return Expanded(child: Padding(padding: const EdgeInsets.symmetric(horizontal: 1), child: Column(mainAxisAlignment: MainAxisAlignment.end, children: [Container(height: 90 * pct, decoration: BoxDecoration(color: const Color(0xFFAB47BC).withValues(alpha:0.6), borderRadius: const BorderRadius.vertical(top: Radius.circular(4)))), const SizedBox(height: 4), Text('${_schedule[i].month}', style: TextStyle(color: Colors.white.withValues(alpha:0.3), fontSize: 7))]))); })])), const SizedBox(height: 12), Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8), decoration: BoxDecoration(color: const Color(0xFFAB47BC).withValues(alpha:0.1), borderRadius: BorderRadius.circular(8)), child: Row(children: [SizedBox(width: 35, child: Text('#', style: TextStyle(color: Colors.white.withValues(alpha:0.4), fontSize: 11, fontWeight: FontWeight.w600))), Expanded(flex: 2, child: Text('Principal', style: TextStyle(color: Colors.white.withValues(alpha:0.4), fontSize: 11, fontWeight: FontWeight.w600))), Expanded(flex: 2, child: Text('Interest', style: TextStyle(color: Colors.white.withValues(alpha:0.4), fontSize: 11, fontWeight: FontWeight.w600))), Expanded(flex: 2, child: Text('Balance', style: TextStyle(color: Colors.white.withValues(alpha:0.4), fontSize: 11, fontWeight: FontWeight.w600)))])), ..._schedule.take(24).map((e) => Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6), decoration: BoxDecoration(border: Border(bottom: BorderSide(color: Colors.white.withValues(alpha:0.03)))), child: Row(children: [SizedBox(width: 35, child: Text('${e.month}', style: const TextStyle(color: Colors.white54, fontSize: 11))), Expanded(flex: 2, child: Text(NumberFormat.compact().format(e.principal), style: const TextStyle(color: Color(0xFF4CAF50), fontSize: 11))), Expanded(flex: 2, child: Text(NumberFormat.compact().format(e.interest), style: const TextStyle(color: Color(0xFFEF5350), fontSize: 11))), Expanded(flex: 2, child: Text(NumberFormat.compact().format(e.balance), style: const TextStyle(color: Colors.white54, fontSize: 11)))]))), if (_schedule.length > 24) Padding(padding: const EdgeInsets.all(12), child: Text('+ ${_schedule.length - 24} more payments...', style: TextStyle(color: Colors.white.withValues(alpha:0.3), fontSize: 11)))], const SizedBox(height: 12), Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: const Color(0xFFAB47BC).withValues(alpha:0.05), borderRadius: BorderRadius.circular(12)), child: Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [_buildMiniStat('Total Payments', '${_schedule.length}'), _buildMiniStat('Avg Monthly', 'LKR ${NumberFormat.compact().format(_totalPayment / _schedule.length)}'), _buildMiniStat('Interest Share', '${(_totalInterest / _totalPayment * 100).toStringAsFixed(1)}%')]))]);
  Widget _buildMiniStat(String l, String v) => Column(children: [Text(v, style: const TextStyle(color: Color(0xFFAB47BC), fontWeight: FontWeight.bold, fontSize: 14)), Text(l, style: TextStyle(color: Colors.white.withValues(alpha:0.4), fontSize: 10))]);
  ...
  Widget _buildIP(String t, String d) => Padding(padding: const EdgeInsets.only(bottom: 10), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [Container(margin: const EdgeInsets.only(top: 4), width: 6, height: 6, decoration: const BoxDecoration(color: Color(0xFFAB47BC), shape: BoxShape.circle)), const SizedBox(width: 10), Expanded(child: RichText(text: TextSpan(children: [TextSpan(text: '$t: ', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w600, fontSize: 13)), TextSpan(text: d, style: TextStyle(color: Colors.white.withValues(alpha:0.65), fontSize: 13, height: 1.5))])))]));

  Widget _buildDisclaimer() => Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: const Color(0xFFFFA726).withValues(alpha:0.05), borderRadius: BorderRadius.circular(16), border: Border.all(color: const Color(0xFFFFA726).withValues(alpha:0.15))), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [const Icon(Icons.warning_amber_rounded, color: Color(0xFFFFA726), size: 20), const SizedBox(width: 10), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Disclaimer', style: TextStyle(color: Color(0xFFFFA726), fontWeight: FontWeight.bold, fontSize: 13)), const SizedBox(height: 4), Text('Rates are indicative. Actual rates depend on credit score, income, and bank relationship. Contact the bank for exact rates and terms.', style: TextStyle(color: Colors.white.withValues(alpha:0.6), fontSize: 11, height: 1.5))]))]));
}
...
class BankFDResult {
  ...
  final double rate, grossInterest, whtAmount, interest, maturity, effectiveAnnualRate;
  final int months;
  BankFDResult({
    ...
    required this.interest,    // net interest (after 5% WHT)
    required this.maturity,    // principal + net interest
